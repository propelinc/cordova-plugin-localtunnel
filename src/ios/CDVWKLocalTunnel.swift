...
@objc(CDVWKLocalTunnel) class CDVWKLocalTunnel : CDVPlugin, WebViewPropagateDelegate {
    ...
    var captchaRequestSession = false;
    ...
    */
    ...
    func open(command: CDVInvokedUrlCommand) {
        ...
        let runOpen = {
            ...
            if self.requestOptions?.requestType == CLEAR_COOKIES_REQUEST {
                ...
                self.captchaRequestSession = true
    ...
    func destroyWebViewController() {
        ...
        // Captcha working depends on the WKWebview not being destroyed between CAPTCHA_REQUEST and subsequent
        // HTTP_REQUEST. That means that we cannot close the webViewController when a captcha request is closed
        if self.requestOptions?.requestType == CAPTCHA_REQUEST {
            ...
            // Once a non CAPTCHA_REQUEST has succeeded we
            self.captchaRequestSession = false
                ...
                if error == nil {
                    ...
                    if 200 <= statusCode && statusCode < 400 {
                        ...
                        self.setCookies(cookies: returnedCookies, completionHandler: {
                            print("Set cookies from URL response \(returnedCookies)")
